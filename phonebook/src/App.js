    ...
    <form onSubmit={props.addName}>
      ...
      <div>
        <Field
          ...
          handleChange={props.numberChange}
        />
      </div>
      <div>
...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if (
      ...
      if (
        window.confirm(
          `${newName} already exists in this phonebook. Do you want to replace their number?`
        )
      ) {
        personService
      ...
      personService
        ...
        .catch((error) => { 
          if (newName.length < 3) {
            setErrorMessage(
              `Person validation failed: Path 'name' (${newName}) is shorter than minimum allowed length (3).`
            );
          }
          setTimeout(() => {
            setErrorMessage(null);
  ...
  const matchingPersons = persons.filter((person) =>
    person.name.toLowerCase().includes(filterdPerson.toLowerCase())
  );
